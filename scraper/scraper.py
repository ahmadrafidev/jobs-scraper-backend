...
def scrape_jobstreet(url, keyword):
    logging.info(f"Scraping Jobstreet for keyword: {keyword}")
    ...
    for job in jobs:
        ...
        logging.info(f"Inserted/Updated job: {title} at {company} for keyword: {keyword}")

# Function to scrape job listings from karir.com
def scrape_karir(keyword):
    logging.info(f"Scraping Karir for keyword: {keyword}")
...
def scrape_kalibrr(keyword):
    logging.info(f"Scraping Kalibrr for keyword: {keyword}")
    ...
    for job in jobs:
        ...
        company = job.find('span', class_='k-text-subdued').text.strip() if job.find('span', 'k-text-subdued') else None
        location = job.find('span', class_='k-job-location').text.strip() if job.find('span', 'k-job-location') else None
        date_posted = job.find('span', class_='k-job-postdate').text.strip() if job.find('span', 'k-job-postdate') else None
        job_link = job.find('a')['href'] if job.find('a') else None
